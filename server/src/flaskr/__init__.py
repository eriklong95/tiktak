...
from src.flaskr.persistence.db_connection.db_connection_supplier import (
...
def initialize_database(logger):
    ...
    if not user_table_exists:
        cursor.execute("CREATE TABLE user(username TEXT PRIMARY KEY, rank INTEGER)")
...
def configure_routing(app):
    ...
    def test():
        return make_response(
            "You called the tiktak server and had a successful response", 200
        )
